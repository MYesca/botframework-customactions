...
{
    public class ModifyTable : Dialog
    {
        private readonly string urlPowerBiServiceApiRoot = "https://api.powerbi.com";
        private PowerBIClient _pbiClient;

        [JsonProperty("$kind")]
        public const string Kind = "ModifyTable";

        [JsonProperty("datasetId")]
        public StringExpression DatasetId { get; set; }

        [JsonProperty("tableDefinition")]
        public ValueExpression TableDefinition { get; set; }

        [JsonProperty("resultProperty")]
        public StringExpression ResultProperty { get; set; }

        [JsonConstructor]
        public ModifyTable([CallerFilePath] string sourceFilePath = "", [CallerLineNumber] int sourceLineNumber = 0)
            : base()
        {
        ...
        public override async Task<DialogTurnResult> BeginDialogAsync(DialogContext dc, object options = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                Table request = TableDefinition?.EvaluateExpression<Table>(dc.State);
                ...
                DoModifyTable(workspaceId, datasetId, request);
        ...
        private void DoModifyTable(Guid workspaceId, string datasetId, Table request)
        {
            Tables tables = _pbiClient.Datasets.GetTablesInGroup(workspaceId, datasetId);
            Table table = tables.Value.FirstOrDefault(d => d.Name == request.Name);
            if (table == null)
            {
                throw new Exception($"Table {request.Name} not found for modifying, aborting.");
            }

            _pbiClient.Datasets.PutTableInGroup(workspaceId, datasetId, request.Name, request);
        }
    }
}
